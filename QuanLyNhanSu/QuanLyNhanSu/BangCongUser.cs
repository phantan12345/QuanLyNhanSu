...
{
    public partial class BangCongUser : Form
    {
        BUS_BangCongUser bangCongUser;
        BUS_NhanVienUser nhanVienUser;
        public BangCongUser()
        {
            InitializeComponent();
            bangCongUser = new BUS_BangCongUser();
            nhanVienUser = new BUS_NhanVienUser();
        }

        public BangCongUser(NhanVien nv) : this()
        {
            dGBangCong.DataSource = null;
            bangCongUser.getBangCong(dGBangCong, nv.MaNV);
        ...
        private void BangCongUser_Load(object sender, EventArgs e)
        {
        ...
        private void btnXem_Click(object sender, EventArgs e)
        {
            ...
            bangCongUser.searchBangCong(dGBangCong, lbMaNv.Text, Int32.Parse(txtThang.Text), Int32.Parse(txtNam.Text));
            txtNam.Text = "";
            txtThang.Text = "";
        }
        ...
        private void btnChamCong_Click(object sender, EventArgs e)
        {
            ...
            bangCong.MaNV = lbMaNv.Text;
            bangCong.NgayLam = currentDate.Day;
            bangCong.ThangLam = currentDate.Month;
            bangCong.NamLam = currentDate.Year;
            bangCong.Active = true;
            bangCong.MaBangCong = "BC10";
            if (bangCongUser.addBangCong(bangCong))
            {
                MessageBox.Show("Chấm công thành công.");
            }
            else
            {
                MessageBox.Show("Bạn đã chấm công.");

            }
            dGBangCong.DataSource = null;
            bangCongUser.getBangCong(dGBangCong, lbMaNv.Text);
